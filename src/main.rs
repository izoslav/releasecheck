...
struct Opts {
    ...
    #[clap(
        short,
        long,
        about = "Comma-separated list of platform short names for filtering"
    )]
    ...
    #[clap(
        long,
        about = "List available platforms and their short names, and exits program"
    )]
    list_platforms: bool,
...
async fn main() {
    ...
    let platforms = opts
        .platforms
        .as_ref()
        .map_or_else(Vec::new, |p| p.split(',').map(|e| e.to_string()).collect());

    let genres = opts.genres.as_ref().map_or_else(Vec::new, |g| {
        g.split(',').map(|e| e.to_string().to_lowercase()).collect()
    });
